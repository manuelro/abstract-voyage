...
export default function App() {
  ...
  // Combined pass rate per palette (Light+Dark together)
  ...
  // Selected palette combined (same as section header summary)
  const activeCombinedRate = activeIsSecondary ? secondaryCombinedRate : primaryCombinedRate;

  // Per-theme breakdown for the selected palette using the same engine:
  // Trick: duplicate the theme's surfaces across both slots so the ratio reflects that theme.
  ...
  // A11y stats used inside the table card (unchanged)
  const stats = useMemo(() => {
    const hex = activeIsSecondary ? secondaryHex : primaryHex;
  ...
  // ---------- Optimizer (unchanged logic; now triggered from top bar) ----------
        ...
        <div id="rightPane">
          <div className="right-stack">
            ...
            {/* <TinyUIPreview
              globalTheme={globalTheme}
              styleVars={styleVars}
              tabTokens={{}} // no per-section theme tabs anymore
            /> */}
            ...
            <A11yAudit
              ...
              targetPassRate={TARGET_PASS_RATE}
              isOptimizing={isOptimizing}
              // Hide in-section optimize button (moved to top bar)
