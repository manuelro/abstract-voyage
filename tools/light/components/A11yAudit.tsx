...
function Table({ scopeClass, scopeVars, steps }) {
  ...
  return (
    <div className={scopeClass} style={scopeVars}>
      <div className="a11y-wrap">
        <table className="a11y" aria-label={`WCAG contrast audit (${scopeClass.includes('light') ? 'light' : 'dark'})`}>
          ...
          <tbody>
            {checks.map((c, i) => (
              <tr key={i}>
                <td className="a11y-use">{c.use}</td>
      ...
      <p className="a11y-note">Text: AA ≥ 4.5, AAA ≥ 7. Large text AA ≥ 3. Non-text UI (borders, focus rings) ≥ 3.</p>
    </div>
  );
}

export default function A11yAudit({
  activeTheme, onThemeChange, styleVars, steps,
  passRate, totalChecks, onOptimizeBase, targetPassRate = 0.95,
  isOptimizing = false
}) {
  const pct = Math.round(passRate * 100);
  const showOptimize = passRate < targetPassRate;
  ...
  return (
    <section>
      ...
      <div className="row" style={{ justifyContent: 'space-between', margin: '8px 0' }}>
        <div className="muted">
          Accent link/focus pass rate: <strong>{pct}%</strong> of {totalChecks} checks
          {' '} (target ≥ {Math.round(targetPassRate*100)}%)
        </div>
        {showOptimize && (
          <button
            className="copy cta"
