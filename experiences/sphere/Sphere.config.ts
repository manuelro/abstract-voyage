...
const SLOW_FACTOR = 3 // ← 100% slower (2× durations)

/* ---- Arrows (per-side) with continuous cross-fade ---- */
const RAW_ENTER = Math.round(BASE * GOLDEN)                // ~262ms
const RAW_HOLD  = Math.round(BASE * Math.sqrt(GOLDEN))     // ~206ms
const RAW_FADE  = Math.round(BASE * Math.pow(GOLDEN, 0.8)) // ~238ms

const ARROWS_ENTER = RAW_ENTER * SLOW_FACTOR               // 524ms
const ARROWS_HOLD  = RAW_HOLD  * SLOW_FACTOR               // 412ms
const ARROWS_FADE  = RAW_FADE  * SLOW_FACTOR               // 476ms

/** Cross-fade overlap (left starts before right finishes) */
const ARROWS_OVERLAP_FRAC = 1 / GOLDEN                     // ~0.618
const ARROWS_OVERLAP_MS   = Math.round(ARROWS_FADE * ARROWS_OVERLAP_FRAC) // ≈ 294ms
const ARROWS_GAP          = -ARROWS_OVERLAP_MS             // negative gap creates continuity

/** Start delay (also slowed 2×) */
const ARROWS_START_DELAY = Math.round(BASE * Math.pow(GOLDEN, 3)) * SLOW_FACTOR // 1372ms

/** Arrow window (2× slower & overlapping) */
const ARROWS_WINDOW = (ARROWS_ENTER + ARROWS_HOLD + ARROWS_FADE) * 2 + ARROWS_GAP // 2530ms

/* ---- Reduced motion (shorter), preserving overlap fraction ---- */
const ARROWS_ENTER_R = Math.round(ARROWS_ENTER / GOLDEN)   // ~324ms
const ARROWS_HOLD_R  = Math.round(ARROWS_HOLD  / GOLDEN)   // ~255ms
const ARROWS_FADE_R  = Math.round(ARROWS_FADE  / GOLDEN)   // ~294ms
const ARROWS_OVERLAP_MS_R = Math.round(ARROWS_FADE_R * ARROWS_OVERLAP_FRAC) // ~182ms
const ARROWS_GAP_R   = -ARROWS_OVERLAP_MS_R

/* ---- Post-arrows reveals ---- */
const DELAY_AFTER_ARROWS_TO_TEXT       = Math.round((BASE / GOLDEN)) * SLOW_FACTOR        // 200ms
const DELAY_AFTER_TEXT_END_TO_CHROME   = Math.round((BASE / Math.pow(GOLDEN, 1.5))) * SLOW_FACTOR // 158ms

/* ---- Content fade durations (these are for page chrome; text fade is the carouselFade duration) ---- */
const TEXT_FADE_MS   = Math.round(BASE * (0.75 + 1 / GOLDEN)) * SLOW_FACTOR // 444ms
const HEADER_FADE_MS = Math.round(BASE * (0.50 + 1 / GOLDEN)) * SLOW_FACTOR // 362ms
const FOOTER_FADE_MS = Math.round(BASE * (0.48 + 1 / GOLDEN)) * SLOW_FACTOR // 356ms
...
const PARAMS: Params = {
  ...
  motion: {
    ...
    carouselFade: {
      ...
      // IMPORTANT: let text reveal closely after the sphere intro; do NOT push it behind arrows.
      overlapMs: 0,
      durationMs: 1600,
...
function toConfig(p: Params): Config {
  ...
  return {
    ...
    behavior: {
      ...
      /* PHI-driven UI orchestration for hint + reveals */
      arrowsHint: {
        zIndex: 60,
...
/** Exported helpers for consumers (DragArrowsHint, page, etc.) */
