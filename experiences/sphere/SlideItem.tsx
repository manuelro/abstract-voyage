...
export type SlideItemProps = { q: string; desc: string; category: string; preferReducedMotion: boolean; style?: React.CSSProperties }

export default function SlideItem({ q, desc, category, preferReducedMotion, style = {} }: SlideItemProps) {
  const transitionValue = preferReducedMotion
    ? 'none'
    : 'opacity var(--fade-duration, 1600ms) var(--fade-ease, ease)'

  return (
    <blockquote style={{ margin: 0, border: 0, boxShadow: 'none', ...style }}>
      <p
        style={{
          margin: 0,
          fontSize: '1.25rem',
