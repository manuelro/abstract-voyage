...
export const POLYMORPHIC_LAYOUT_FIELDS: ReadonlyArray<ConfigScopeEntry<PolymorphicLayoutConfig>> = [
    ...
    // layout is being edited, so neither area collapses it.
    {
      ...
      areas: [
        ...
        {
          // Root-level area, a peer of HEADER/CONTENT above — not nested
          // inside either. scrollGradientEnabled(-Wide/-Lg) is an
          // independent override that, when on, takes over both columns'
          // paint/ink AND renders behind the header row too (see
          // PolymorphicLayout.tsx's own scrollGradientActive usage) —
          // living under CONTENT would have misrepresented that scope the
          // same way COLUMN_COLOR_KEYS's own doc comment above describes
          // for the "ALL SIZES vs MOBILE" base-tier mistake. See
              ...
              tabs: [
                {
                  ...
                  fields: [
                    {
                      kind: 'number',
                      key: 'scrollGradientTauMs',
                      label: 'Scroll gradient smoothing',
                      description: 'Exponential-smoothing time constant (ms) for the darken overlay — shared across all three breakpoint tiers, not tiered itself.',
                      min: 0,
                      max: 5000,
                      step: 10,
                      unit: 'ms',
                      integer: true,
                      visibleWhen: config => (
                        config.scrollGradientEnabled
                        || config.scrollGradientEnabledWide
                    ...
                    },
                    {
                      ...
                      fields: [
                        {
                          kind: 'boolean',
                          key: 'wordmarkUsesScrollGradient',
                          label: 'Wordmark uses scroll gradient',
                          description: 'Opt-in, off by default. When on (and the currently-active tier\'s own Scroll gradient background is also on), the wordmark\'s own gradient is derived from that same tier\'s hue/scheme/chroma/variance/seed, plus the independent shape controls below, instead of its normal WordmarkConfig color. See PLAN-WORDMARK-SCROLL-GRADIENT-INTEGRATION.md.',
                        },
                        {
                          kind: 'enum',
                          key: 'wordmarkGradientClarity',
                          label: 'Clarity direction',
                          description: 'Auto: reads light vs. dark from the active tier\'s own Ink color (MOBILE/TABLET/DESKTOP tabs below) — light uses Lightness min/max below as authored, dark mirrors them (100-max/100-min). Light/Dark force a direction regardless of Ink color.',
                          options: [
                            { label: 'AUTO (from ink color)', value: 'auto' },
                        ...
                        {
                          kind: 'number',
                          key: 'wordmarkGradientLightnessMax',
                          label: 'Lightness max',
                          description: 'Authored for the LIGHT clarity direction; mirrored (100-min) for DARK. Paired with the tiered Lightness min in each of the MOBILE/TABLET/DESKTOP tabs\' own Wordmark group below — not itself tiered, since one ceiling works across breakpoints. Independent of the background\'s own lightness.',
                          min: 0,
                          max: 100,
                          step: 1,
                          integer: true,
                        ...
                        {
                          kind: 'number',
                          key: 'wordmarkGradientHueSpread',
                          label: 'Hue spread',
                          description: 'Degrees of hue jitter/band width around whichever base hue is active — a dimension the background feature never exposes at all. Not tiered — applies across every breakpoint.',
                          min: 0,
                          max: 90,
                          step: 1,
                          unit: 'deg',
                        ...
                        {
                          kind: 'number',
                          key: 'wordmarkGradientZoom',
                          label: 'Zoom',
                          description: '0: every stop converges to the center color. 1: full variation. A dimension the background feature never exposes at all. Not tiered — applies across every breakpoint.',
                          min: 0,
                          max: 1,
                          step: 0.05,
                          visibleWhen: config => config.wordmarkUsesScrollGradient,
                        ...
                        {
                          kind: 'number',
                          key: 'wordmarkGradientDarken',
                          label: 'Darken',
                          description: '0-1 post-hoc lightness multiplier applied to every generated stop after Clarity/Lightness/Chroma have already picked it — the wordmark\'s own analog to the background\'s Max darken (which composites a black overlay div the wordmark has no equivalent layer for). 0 leaves stops untouched; 1 renders fully black regardless of Lightness min/max.',
                          min: 0,
                          max: 1,
                          step: 0.05,
                          visibleWhen: config => config.wordmarkUsesScrollGradient,
                  ...
                  fields: [
                    {
                      ...
                      fields: [
                        {
                          kind: 'number',
                          key: 'wordmarkGradientBaseHue',
                          label: 'Hue',
                          description: 'Main hue (0-360) the wordmark\'s own procedural gradient is built around — independent of the background\'s own Hue.',
                          min: 0,
                          max: 360,
                          step: 1,
                          unit: '°',
                        ...
                        {
                          kind: 'enum',
                          key: 'wordmarkGradientHueScheme',
                          label: 'Hue scheme',
                          description: 'Mono: single hue band around the hue above. Dual complementary: two opposing hues bridged through a softer center. Independent of the background\'s own Hue scheme.',
                          options: [
                            { label: 'MONO', value: 'mono' },
                            { label: 'DUAL COMPLEMENTARY', value: 'dual-complementary' },
                          ],
                          visibleWhen: config => config.wordmarkUsesScrollGradient,
                        },
                        {
                          kind: 'number',
                          key: 'wordmarkGradientLightnessMin',
                          label: 'Lightness min',
                          description: 'Authored for the LIGHT clarity direction (ALL SIZES tab above); mirrored (100-max) for DARK. Paired with the ALL SIZES tab\'s own (non-tiered) Lightness max. Independent of the background\'s own Lightness min.',
                          min: 0,
                          max: 100,
                          step: 1,
                          visibleWhen: config => config.wordmarkUsesScrollGradient,
                        ...
                        {
                          kind: 'number',
                          key: 'wordmarkGradientChromaMin',
                          label: 'Chroma min',
                          description: 'Independent of the background\'s own Chroma min.',
                          min: 0,
                          max: 100,
                          step: 1,
                          visibleWhen: config => config.wordmarkUsesScrollGradient,
                        ...
                        {
                          kind: 'enum',
                          key: 'wordmarkGradientMode',
                          label: 'Distribution',
                          description: 'Side bright: dark center, bright edges. Center bright: bright center, darker edges. Independent of the background\'s own Distribution.',
                          options: [
                            { label: 'SIDE BRIGHT', value: 'side-bright' },
                            { label: 'CENTER BRIGHT', value: 'center-bright' },
                          ],
                          visibleWhen: config => config.wordmarkUsesScrollGradient,
                        },
                        {
                          kind: 'number',
                          key: 'wordmarkGradientStops',
                          label: 'Stop count',
                          description: 'Independent of the background\'s own Stop count — Logo.tsx\'s own default wordmark gradient uses 22.',
                          min: 2,
                          max: 64,
                          step: 1,
                          integer: true,
                        ...
                        {
                          kind: 'number',
                          key: 'wordmarkGradientVariance',
                          label: 'Variance',
                          description: '0 = almost fixed pattern, 1 = maximum (still bounded) variation. Independent of the background\'s own Variance.',
                          min: 0,
                          max: 1,
                          step: 0.01,
                          visibleWhen: config => config.wordmarkUsesScrollGradient,
                        ...
                        {
                          kind: 'number',
                          key: 'wordmarkGradientCenterStretch',
                          label: 'Center stretch',
                          description: 'Warps the color distribution toward the center — 0 is no stretch, 1 is a strongly dominant middle band. Independent of the background\'s own Center stretch.',
                          min: 0,
                          max: 1,
                          step: 0.01,
                          visibleWhen: config => config.wordmarkUsesScrollGradient,
                        ...
                        {
                          kind: 'number',
                          key: 'wordmarkGradientSeed',
                          label: 'Seed',
                          description: 'Deterministic — the same seed always renders the same palette. Independent of the background\'s own Seed.',
                          min: 0,
                          max: 100000,
                          step: 1,
                          integer: true,
                    ...
                    },
                    {
                      ...
                      fields: [
                        ...
                        },
                        {
                          ...
                          label: 'Hue',
                          description: 'Main hue (0-360) the procedural gradient is built around.',
                          min: 0,
                          max: 360,
                          step: 1,
                          unit: '°',
                        ...
                        {
                          kind: 'enum',
                          key: 'scrollGradientHueScheme',
                          label: 'Hue scheme',
                          description: 'Mono: single hue band around the hue above. Dual complementary: two opposing hues bridged through a softer center.',
                          options: [
                            { label: 'MONO', value: 'mono' },
                            { label: 'DUAL COMPLEMENTARY', value: 'dual-complementary' },
                          ],
                        ...
                        {
                          kind: 'number',
                          key: 'scrollGradientLightnessMin',
                          label: 'Lightness min',
                          min: 0,
                          max: 100,
                          step: 1,
                        ...
                        {
                          kind: 'enum',
                          key: 'scrollGradientMode',
                          label: 'Distribution',
                          description: 'Side bright: dark center, bright edges. Center bright: bright center, darker edges.',
                          options: [
                            { label: 'SIDE BRIGHT', value: 'side-bright' },
                            { label: 'CENTER BRIGHT', value: 'center-bright' },
                          ],
                        ...
                        {
                          kind: 'number',
                          key: 'scrollGradientVariance',
                          label: 'Variance',
                          description: '0 = almost fixed pattern, 1 = maximum (still bounded) variation.',
                          min: 0,
                          max: 1,
                          step: 0.01,
                          visibleWhen: config => config.scrollGradientEnabled,
                        ...
                        {
                          kind: 'number',
                          key: 'scrollGradientCenterStretch',
                          label: 'Center stretch',
                          description: 'Warps the color distribution toward the center — 0 is no stretch, 1 is a strongly dominant middle band.',
                          min: 0,
                          max: 1,
                          step: 0.01,
                          visibleWhen: config => config.scrollGradientEnabled,
                        ...
                        {
                          kind: 'number',
                          key: 'scrollGradientSeed',
                          label: 'Seed',
                          description: 'Deterministic — the same seed always renders the same palette.',
                          min: 0,
                          max: 100000,
                          step: 1,
                          integer: true,
                        ...
                        {
                          kind: 'color',
                          key: 'scrollGradientInkColor',
                          label: 'Ink color',
                          description: 'The text/contrast basis every downstream reader of this column\'s color (nav, headings, body copy) derives from while the scroll gradient is active — independent of the palette above. Also what MobilePinnedArticleSection\'s own carousel/panel paint resolves to, unless that value is itself allowed to go transparent — see wideColumnPaintColor.',
                          visibleWhen: config => config.scrollGradientEnabled,
                        },
                        {
                          kind: 'number',
                          key: 'scrollGradientViewportRangeVh',
                          label: 'Darken range',
                          description: 'Scroll distance, in viewport-heights, over which the darken overlay ramps from 0 to its max.',
                          min: 0.1,
                          max: 10,
                          step: 0.05,
                          unit: 'vh×',
                        ...
                        },
                        {
                          ...
                          label: 'Max darken',
                          description: 'Ceiling opacity of the scroll-driven black overlay.',
                          min: 0,
                          max: 1,
                          step: 0.01,
                          visibleWhen: config => config.scrollGradientEnabled,
                ...
                },
                {
                  ...
                  fields: [
                    {
                      kind: 'group',
                      label: 'Wordmark (≥ tablet)',
                      fields: [
                        {
                          kind: 'number',
                          key: 'wordmarkGradientBaseHueWide',
                          label: 'Hue (≥ tablet)',
                          description: 'Overrides the Mobile tab\'s own Wordmark Hue starting at md.',
                          min: 0,
                          max: 360,
                          step: 1,
                          unit: '°',
                        ...
                        {
                          kind: 'enum',
                          key: 'wordmarkGradientHueSchemeWide',
                          label: 'Hue scheme (≥ tablet)',
                          options: [
                            { label: 'MONO', value: 'mono' },
                            { label: 'DUAL COMPLEMENTARY', value: 'dual-complementary' },
                          ],
                        ...
                        {
                          kind: 'number',
                          key: 'wordmarkGradientLightnessMinWide',
                          label: 'Lightness min (≥ tablet)',
                          min: 0,
                          max: 100,
                          step: 1,
                          visibleWhen: config => config.wordmarkUsesScrollGradient,
                        ...
                        {
                          kind: 'number',
                          key: 'wordmarkGradientChromaMinWide',
                          label: 'Chroma min (≥ tablet)',
                          min: 0,
                          max: 100,
                          step: 1,
                          visibleWhen: config => config.wordmarkUsesScrollGradient,
                        ...
                        {
                          kind: 'enum',
                          key: 'wordmarkGradientModeWide',
                          label: 'Distribution (≥ tablet)',
                          options: [
                            { label: 'SIDE BRIGHT', value: 'side-bright' },
                            { label: 'CENTER BRIGHT', value: 'center-bright' },
                          ],
                        ...
                        {
                          kind: 'number',
                          key: 'wordmarkGradientStopsWide',
                          label: 'Stop count (≥ tablet)',
                          min: 2,
                          max: 64,
                          step: 1,
                          integer: true,
                        ...
                        {
                          kind: 'number',
                          key: 'wordmarkGradientVarianceWide',
                          label: 'Variance (≥ tablet)',
                          min: 0,
                          max: 1,
                          step: 0.01,
                          visibleWhen: config => config.wordmarkUsesScrollGradient,
                        ...
                        {
                          kind: 'number',
                          key: 'wordmarkGradientCenterStretchWide',
                          label: 'Center stretch (≥ tablet)',
                          min: 0,
                          max: 1,
                          step: 0.01,
                          visibleWhen: config => config.wordmarkUsesScrollGradient,
                    ...
                    },
                    {
                      ...
                      fields: [
                        ...
                        {
                          kind: 'enum',
                          key: 'scrollGradientHueSchemeWide',
                          label: 'Hue scheme (≥ tablet)',
                          options: [
                            { label: 'MONO', value: 'mono' },
                            { label: 'DUAL COMPLEMENTARY', value: 'dual-complementary' },
                        ...
                        {
                          kind: 'number',
                          key: 'scrollGradientLightnessMinWide',
                          label: 'Lightness min (≥ tablet)',
                          min: 0,
                          max: 100,
                          step: 1,
                        ...
                        {
                          kind: 'number',
                          key: 'scrollGradientChromaMinWide',
                          label: 'Chroma min (≥ tablet)',
                          min: 0,
                          max: 100,
                          step: 1,
                        ...
                        {
                          kind: 'number',
                          key: 'scrollGradientVarianceWide',
                          label: 'Variance (≥ tablet)',
                          min: 0,
                          max: 1,
                          step: 0.01,
                        ...
                        {
                          kind: 'number',
                          key: 'scrollGradientCenterStretchWide',
                          label: 'Center stretch (≥ tablet)',
                          min: 0,
                          max: 1,
                          step: 0.01,
                        ...
                        {
                          kind: 'color',
                          key: 'scrollGradientInkColorWide',
                          label: 'Ink color (≥ tablet)',
                          visibleWhen: config => config.scrollGradientEnabledWide,
                        },
                        {
                          kind: 'number',
                          key: 'scrollGradientViewportRangeVhWide',
                          label: 'Darken range (≥ tablet)',
                          min: 0.1,
                          max: 10,
                          step: 0.05,
                          unit: 'vh×',
                ...
                },
                {
                  ...
                  fields: [
                    {
                      kind: 'group',
                      label: 'Wordmark (≥ desktop)',
                      fields: [
                        {
                          kind: 'number',
                          key: 'wordmarkGradientBaseHueLg',
                          label: 'Hue (≥ desktop)',
                          description: 'Overrides the Tablet tab\'s own Wordmark Hue (≥ tablet) starting at 1024px.',
                          min: 0,
                          max: 360,
                          step: 1,
                          unit: '°',
                        ...
                        {
                          kind: 'enum',
                          key: 'wordmarkGradientHueSchemeLg',
                          label: 'Hue scheme (≥ desktop)',
                          options: [
                            { label: 'MONO', value: 'mono' },
                            { label: 'DUAL COMPLEMENTARY', value: 'dual-complementary' },
                          ],
                        ...
                        {
                          kind: 'number',
                          key: 'wordmarkGradientLightnessMinLg',
                          label: 'Lightness min (≥ desktop)',
                          min: 0,
                          max: 100,
                          step: 1,
                          visibleWhen: config => config.wordmarkUsesScrollGradient,
                        ...
                        {
                          kind: 'number',
                          key: 'wordmarkGradientChromaMinLg',
                          label: 'Chroma min (≥ desktop)',
                          min: 0,
                          max: 100,
                          step: 1,
                          visibleWhen: config => config.wordmarkUsesScrollGradient,
                        ...
                        {
                          kind: 'number',
                          key: 'wordmarkGradientStopsLg',
                          label: 'Stop count (≥ desktop)',
                          min: 2,
                          max: 64,
                          step: 1,
                          integer: true,
                        ...
                        {
                          kind: 'number',
                          key: 'wordmarkGradientVarianceLg',
                          label: 'Variance (≥ desktop)',
                          min: 0,
                          max: 1,
                          step: 0.01,
                          visibleWhen: config => config.wordmarkUsesScrollGradient,
                        ...
                        {
                          kind: 'number',
                          key: 'wordmarkGradientCenterStretchLg',
                          label: 'Center stretch (≥ desktop)',
                          min: 0,
                          max: 1,
                          step: 0.01,
                          visibleWhen: config => config.wordmarkUsesScrollGradient,
                    ...
                    },
                    {
                      ...
                      fields: [
                        ...
                        {
                          kind: 'number',
                          key: 'scrollGradientLightnessMinLg',
                          label: 'Lightness min (≥ desktop)',
                          min: 0,
                          max: 100,
                          step: 1,
                        ...
                        {
                          kind: 'number',
                          key: 'scrollGradientCenterStretchLg',
                          label: 'Center stretch (≥ desktop)',
                          min: 0,
                          max: 1,
                          step: 0.01,
                        ...
                        {
                          kind: 'color',
                          key: 'scrollGradientInkColorLg',
                          label: 'Ink color (≥ desktop)',
                          visibleWhen: config => config.scrollGradientEnabledLg,
                        },
                        {
                          kind: 'number',
                          key: 'scrollGradientViewportRangeVhLg',
                          label: 'Darken range (≥ desktop)',
                          min: 0.1,
                          max: 10,
                          step: 0.05,
                          unit: 'vh×',
